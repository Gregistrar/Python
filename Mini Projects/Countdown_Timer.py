"""
Countdown Timer (Beginner Level)
================================

- Using data and time functions without built-in generators
- Prodive a continuously decrementing display of the months, days,
    hours, minutes and seconds entered by the user

- User Stories
    + User can see an event input box w/ event name field, date field,
        an optional time, and 'Start' button
    + User can enter the date and event name, if no time is input then
        midnight of the event date is the start time
    + User gets a warning message if the event name is blank
    + User gets a warning message if the event date or time is incorrect
    + User gets a warning message if the time until the event would overflow
        the precision of the countdown timer
    + User can click the 'Start' button to see the countdown timer
    + User can see the elements in the countdown timer automatically decrement
        by the second field

Bonus Features
- User can save the event so it persists across sessions
- User can see an alert when the event is reached
- User can specify more than one event
- User can see a countdown timer for each event that has been added
"""
